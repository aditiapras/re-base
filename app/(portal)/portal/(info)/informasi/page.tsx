...
export default function Page() {
    ...
    const informationPosts = MOCK_POSTS.filter((post) => post.type === "informasi");

    // Calculate Stats
    const totalSemua = informationPosts.length;
    const totalPublished = informationPosts.filter((p) => p.isPublished).length;
    const totalDraft = informationPosts.filter((p) => !p.isPublished).length;
            ...
            <div className="border-b grid grid-cols-3">
                <div className="flex flex-col gap-4 p-4 border-r">
                    <div className="flex items-center gap-2">
                        ...
                        <p className="text-sm font-semibold">Total Informasi</p>
                    </div>
